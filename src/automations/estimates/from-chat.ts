...
async function extractServiceItems(scope: string): Promise<Array<{ description: string; quantity: number; unitPrice: number }>> {
  ...
  const msg = await client.messages.create({
    ...
    system: [
      ...
      '',
      'CRITICAL RULES:',
      '1. Brand/model names (Square D QO, Eaton BR, Leviton, etc.) are SPECS of an item — never extract them as separate line items.',
      '   "Square D QO 40-space panel" → one item: "200A Panel Enclosure", NOT two items.',
      '   "Eaton BR 200A 40-space" → "200A Panel Enclosure"',
      '2. For footage-based items (conduit, wire, cable, trench), set quantity = the linear footage.',
      '   "40 ft of 2/0 AL SER" → {"description":"2/0 AL SER Cable","quantity":40,"unitPrice":0}',
      '   "45 feet of 2.5 inch PVC" → {"description":"2.5\\" PVC Schedule 40 Conduit, per ft","quantity":45,"unitPrice":0}',
      '3. Default panel brand is Eaton BR — include amperage but not brand in the description.',
      '',
      'Examples:',
      '"replace GFCI" → {"description":"Replace GFCI Receptacle","quantity":1,"unitPrice":0}',
      '"200A panel upgrade" → {"description":"200A Panel Enclosure","quantity":1,"unitPrice":0}',
      ...
      '"40 ft 2/0 AL SER cable" → {"description":"2/0 AL SER Cable, per ft","quantity":40,"unitPrice":0}',
      '"install 2.5 inch PVC conduit 40 feet" → {"description":"Install Conduit (1.25\\"-2\\")","quantity":40,"unitPrice":0}',
      '',
