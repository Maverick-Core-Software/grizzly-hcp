...
function clientFor(parentAccount: () => Promise<{ statusCode: number }>, parentNumbers: () => Promise<{ statusCode: number }>) {
  return {
    request: async ({ uri }: { uri: string }) => uri.endsWith('/IncomingPhoneNumbers.json') ? parentNumbers() : parentAccount(),
    api: { v2010: { accounts: () => ({ incomingPhoneNumbers: { list: async () => [] } }) } },
  };
}

async function run(): Promise<void> {
  const denied = await negativeAuthority(clientFor(async () => ({ statusCode: 401 }), async () => ({ statusCode: 403 })), 'ACsubaccount', 'ACparent');
  ...
  assert.equal(inconclusive.pass, false); assert.equal(inconclusive.parentAccountStatus, null, 'generic rejections are inconclusive rather than denials');
