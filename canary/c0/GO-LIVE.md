...
| 3 | Capture production guard before any canary mutation: `npx tsx canary/c0/provision/snapshot-production-guard.ts --production-number-sid <CARTER_PRODUCTION_NUMBER_SID>` | `inventory` | `snapshot-production-guard` evidence has masked SID plus voice/SMS URL SHA-256 hashes. Record this evidence path and hashes for step 15. | No Carter-supplied production SID or guard capture failure. |
...
| 12 | Start only canary PM2 apps: `pm2 start canary/c0/ecosystem.c0.config.cjs --only c0-agent,c0-detector,c0-monitor`; verify with `pm2 ls | Select-String 'c0-agent|c0-detector|c0-monitor'`. `c0-agent` runs the package `tsx` CLI with `canary/c0/agent/src/main.ts start` and a worktree-root cwd. | `agent` | Filtered PM2 output contains only the three `c0-*` applications and all are online; do not run unfiltered start/restart commands. If startup reports a redacted `c0_env_inherited_conflict_<KEY>`, unset that inherited `VOICE_C0_*` (or `VOICE_OUTBOX_PATH`) value in the PM2/shell environment; do not copy or compare secret values manually. | Any production app appears, PM2 starts an unapproved app, a C0 app is not online, or the inherited-conflict error remains after the inherited variable is removed. |
| 13 | After Carter authorizes the enabled transition, run `npx tsx canary/c0/provision/set-enabled.ts --apply --value true`; it prints but does not run `pm2 restart c0-agent`, so run that exact restart command separately. | `agent` | Redacted `set-enabled` evidence has before/after/at and named write only; fresh `preflight --stage agent` evidence passes and filtered PM2 output shows only `c0-agent` restarted. | Enabled writer fails, any key other than `VOICE_C0_ENABLED` changes, or any non-C0 app is restarted. |
...
| 15 | Re-run the same production guard command from step 3 and compare the two recorded hashes exactly. | `enable` | Second `snapshot-production-guard` evidence has the same voice/SMS hashes as step 3. If equal, status is **READY FOR REHEARSAL**, not general availability. | Any hash differs; immediately use the kill switch and investigate without further activation. |

## Kill switch and rollback

**Kill switch (expected effect: new calls use humans within 60 seconds):** `npx tsx canary/c0/provision/twilio-number-route.ts --apply --to fallback`. Confirm the evidence says only `voice_url` changed, then place a new staff call and verify the fallback screen. This does not change a production number.

**Full rollback:** (1) execute the kill switch; (2) `pm2 stop c0-agent c0-detector c0-monitor`; (3) `pm2 delete c0-agent c0-detector c0-monitor`; (4) preserve the C0 evidence, logs, outbox, and marker files for review; (5) leave the deployed Functions service, Sync documents, LiveKit trunk/rule, and subaccount number inert on `/fallback`. Do not delete cloud objects or restore ingress without a new explicit approval and rehearsal.
