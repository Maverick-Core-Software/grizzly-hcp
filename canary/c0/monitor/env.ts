...
export function parseMonitorEnv(source: string, inherited: NodeJS.ProcessEnv = process.env): C0Env {
...
export function loadMonitorEnv(envPath: string, inherited: NodeJS.ProcessEnv = process.env): C0Env {
  try {
    return parseMonitorEnv(readFileSync(envPath, 'utf8'), inherited);
  } catch (error) {
