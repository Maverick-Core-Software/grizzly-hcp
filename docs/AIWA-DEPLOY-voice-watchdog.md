...
Public DNS queries Cloudflare (`1.1.1.1`), Google (`8.8.8.8`), Quad9 (`9.9.9.9`), and OpenDNS (`208.67.222.222`) independently for each A/AAAA lookup, then unions their answers. The redundancy is intentional: Cloudflare returned NXDOMAIN for the Funnel host on AIWA on 2026-09-23 while the other public resolvers supplied its A record. If all resolvers return no record, or no IPv4 public address can be verified, that is a public-path failure and alerts after two consecutive runs; the alert gives the exact reason. Recovery requires a successful IPv4 TwiML and WebSocket probe.
